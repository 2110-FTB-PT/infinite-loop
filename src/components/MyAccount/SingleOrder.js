      ...
      <Link to="/myaccount"><h1>Back To My Account</h1></Link>
            <div className="product-container">
                <div>
                    <div>
                        ...
                        {myOrder.first_name && myOrder.last_name ? <p>Customer: {myOrder.first_name} {myOrder.last_name}</p> : <p>Customer: Guest </p>}
                        <p>Address: {myOrder.address}</p>
                    </div>
                    <h3>Order Details</h3>
                    {myOrder.products && myOrder.products.map((product) => {
            ...
            <form className="edit-product-container" onSubmit={() => handleCancel(id)}>
                ...
                <select value={myOrder.currentStatus} >
                        ...
                        <option value="canceled" onChange={(event) => { setMyOrder({ ...myOrder, currentStatus: event.target.value})}}>Cancel Order</option>
                </select>
                <button>Save</button>
            </form>
    </div>
